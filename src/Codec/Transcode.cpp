...
namespace mediakit
{
            ...
            return _merger.inputFrame(frame, [this, live] (uint64_t dts, uint64_t pts, const Buffer::Ptr& buffer, bool have_idr)
                {
                    ...
                    /* {
                        // 打开一个文件以进行写入
                        std::ofstream outFile("output.h264", std::ios::app); // 以二进制模式打开文件
    ...
    bool FFmpegDecoder::decodeFrame(const char* data, size_t size, uint64_t dts, uint64_t pts, bool live, bool key_frame)
    {
        ...
        // WarnL << "decodeFrame before alloc_av_packet";
        ...
        if (key_frame)
        {
            ...
            // WarnL << "decodeFrame key_frame";
        }

        {
            _context.get()->pix_fmt = AV_PIX_FMT_NV12;

        }

        // WarnL << "decodeFrame before avcodec_send_packet";

        /* {
            // 检查 _context.get() 是否为空
            if (!_context.get()) {
                WarnL << "AVCodecContext is null.";
                return false;
            }

            // 检查编解码器是否已打开
            if (!_context.get()->codec) {
                WarnL << "Codec is not set in AVCodecContext.";
                return false;
            }

            // 打印一些关键信息
            std::cout << "Codec: " << _context.get()->codec->name << std::endl;
            std::cout << "Codec ID: " << _context.get()->codec_id << std::endl;
            std::cout << "Codec Pix fmt: " << _context.get()->pix_fmt << std::endl;

            // 检查编解码器类型，并打印相关参数
            if (_context.get()->codec_type == AVMEDIA_TYPE_VIDEO) {
                std::cout << "Width: " << _context.get()->width << std::endl;
                std::cout << "Height: " << _context.get()->height << std::endl;
            }
            else if (_context.get()->codec_type == AVMEDIA_TYPE_AUDIO) {
                std::cout << "Sample Rate: " << _context.get()->sample_rate << std::endl;
                std::cout << "Channels: " << _context.get()->channels << std::endl;
            }
            else {
                WarnL << "Unsupported codec type: " << _context.get()->codec_type;
                return false;
            }
        } */
        ...
        if (ret < 0)
        {
            ...
            {
                WarnL << "avcodec_send_packet failed:" << ffmpeg_err(ret) << " " << ret;
            }
            return false;
        }
    ...
    FFmpegFrame::Ptr FFmpegSws::inputFrame(const FFmpegFrame::Ptr& frame, int& ret, uint8_t* data)
    {
        ...
        if (!_ctx)
        {
            ...
            _ctx = sws_getContext(
                ...
                (enum AVPixelFormat) frame->get()->format,
                target_width, target_height, _target_format, SWS_FAST_BILINEAR, NULL, NULL, NULL);
            InfoL << "sws_getContext:" << av_get_pix_fmt_name((enum AVPixelFormat) frame->get()->format) << " -> " << av_get_pix_fmt_name(_target_format);
        }
        if (_ctx)
        {
            // WarnL << "FFmpegSws::inputFrame·1";
            auto out = std::make_shared<FFmpegFrame>();
            // WarnL << "FFmpegSws::inputFrame·2";
            ...
            // WarnL << "FFmpegSws::inputFrame·8";
            // {
            //     WarnL << "FFmpegSws::inputFrame·9";
            //     if (!_ctx) {
            //         // Log and handle error
            //         WarnL << "FFmpegSws::inputFrame·!_ctx";
            //         return nullptr;
            //     }
            //     WarnL << "FFmpegSws::inputFrame·10";
            //     if (!frame->get()->data[0] || !out->get()->data[0]) {
            //         // Log and handle error
            //         WarnL << "FFmpegSws::inputFrame·!frame->get()->data[0] || !out->get()->data[0]";
            //         return nullptr;
            //     }
            //     WarnL << "FFmpegSws::inputFrame·11";
            //     if (!frame->get()->linesize[0] || !out->get()->linesize[0]) {
            //         // Log and handle error
            //         WarnL << "FFmpegSws::inputFrame·!frame->get()->linesize || !out->get()->linesize";
            //         return nullptr;
            //     }
            //     WarnL << "FFmpegSws::inputFrame·12";
            //     if (frame->get()->height <= 0 || target_height <= 0) {
            //         // Log and handle error
            //         WarnL << "FFmpegSws::inputFrame·frame->get()->height <= 0 || target_height <= 0";
            //         return nullptr;
            //     }
            //     WarnL << "FFmpegSws::inputFrame·13";

            // }

            // WarnL << "FFmpegSws::inputFrame·in";
            if (0 >= (ret = sws_scale(
                _ctx,
                frame->get()->data,
                frame->get()->linesize,
